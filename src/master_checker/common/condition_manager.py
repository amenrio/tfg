"""ConditionManager is a class to count and store elements meeting a condition
...
class ConditionErrorCriteria(Enum):
    """Error criteria. This enumeration is used when calling ConditionManager.set_error_level
    """
...
class ConditionManager():
    ...
    def __init__(self, name="", display_name="", tooltip="", selectable=True, property_flag = PROPERTY_NONE ):
        """Constructor

        Args:
            name (str, optional): Short name (ID, key) of the condition. Defaults to "".
            display_name (str, optional): Condition name used to display on UI. Defaults to "".
            ...
            property_flag (optional): Type of properties for the element. Default none.
        """
        ...
        """
        self.error_level = ConditionErrorLevel.NONE
        """Error level
        """
        self.elms = []
    ...
    def set_error_level(self, crit):
        ...
        if crit == ConditionErrorCriteria.ERROR_WHEN_NOT_ZERO:
            if self.count != 0:
                self.error_level = ConditionErrorLevel.ERROR
            else:
                self.error_level = ConditionErrorLevel.OK
            ...
            if self.count != 0:
                self.error_level = ConditionErrorLevel.WARN
            else:
                self.error_level = ConditionErrorLevel.OK
